...
- Meminta user di luar accessible_user_ids -> 403, bukan hasil separuh
...
def add_readings(
    db, user_id: uuid.UUID, values: list[tuple[datetime, float]], metric="heart_rate"
) -> None:
...
@pytest.fixture
def keluarga(client, db_session, now):
    """Ayah (admin), ibu (member), dan orang luar."""
    ...
    add_readings(
        db_session, ibu["id"], [(now - timedelta(days=1), 85.0)]
    )
    return {"family": family, "ayah": ayah, "ibu": ibu, "luar": luar}
...
class TestTrend:
    def test_returns_buckets(self, client, keluarga, now) -> None:
        response = client.get(
            TREND,
            params={
    ...
    def test_buckets_are_chronological(self, client, keluarga, now) -> None:
        buckets = client.get(
            TREND,
            params={
                "metric_type": "heart_rate",
    ...
    def test_bucket_carries_aggregates(self, client, keluarga, now) -> None:
        buckets = client.get(
            TREND,
            params={
                "metric_type": "heart_rate",
    ...
    def test_empty_periods_are_omitted(self, client, keluarga, now) -> None:
    ...
    def test_range_is_respected(self, client, keluarga, now) -> None:
        buckets = client.get(
            TREND,
            params={
                "metric_type": "heart_rate",
    ...
    def test_bucket_sizes_accepted(self, client, keluarga, now, bucket) -> None:
        response = client.get(
            TREND,
            params={
                "metric_type": "heart_rate",
    ...
    def test_unknown_metric_rejected(self, client, keluarga, now) -> None:
    ...
    def test_invalid_bucket_rejected(self, client, keluarga, now) -> None:
        response = client.get(
            TREND,
            params={
                "metric_type": "heart_rate",
    ...
    def test_end_before_start_rejected(self, client, keluarga, now) -> None:
        response = client.get(
            TREND,
            params={
                "metric_type": "heart_rate",
...
class TestTrendVisibility:
    def test_can_read_family_member(self, client, keluarga, now) -> None:
        response = client.get(
            TREND,
            params={
                "metric_type": "heart_rate",
    ...
    def test_outsider_forbidden(self, client, keluarga, now) -> None:
        ...
        response = client.get(
            ...
            params={
                ...
                "start": iso(now - timedelta(days=7)),
                "end": iso(now + timedelta(days=1)),
                "user_id": str(keluarga["ayah"]["id"]),
            },
            headers=keluarga["luar"]["headers"],
        )
        assert response.status_code == 403

    def test_private_member_forbidden(self, client, keluarga, now) -> None:
        client.put(
    ...
    def test_forbidden_not_partial_result(self, client, keluarga, now) -> None:
...
class TestSummary:
    def test_returns_metrics(self, client, keluarga, now) -> None:
        response = client.get(
            SUMMARY,
            params={
                "start": iso(now - timedelta(days=7)),
    ...
    def test_aggregates_are_correct(self, client, keluarga, now, db_session) -> None:
    ...
    def test_baseline_included_when_active(
        self, client, keluarga, now, db_session
    ) -> None:
        db_session.add(
            Baseline(
    ...
    def test_baseline_absent_reads_as_collecting(self, client, keluarga, now) -> None:
    ...
    def test_previous_period_comparison(self, client, keluarga, now, db_session) -> None:
    ...
    def test_no_previous_data_gives_null(self, client, keluarga, now) -> None:
        metrics = client.get(
            SUMMARY,
            params={
                "start": iso(now - timedelta(days=7)),
...
class TestFamilyDashboard:
    def test_lists_visible_members(self, client, keluarga) -> None:
        response = client.get(
        ...
        assert names == {"Ayah", "Ibu"}

    def test_shows_latest_reading(self, client, keluarga) -> None:
        members = client.get(
            f"{FAMILIES}/{keluarga['family']['id']}/dashboard",
            headers=keluarga["ayah"]["headers"],
        ).json()["members"]
    ...
    def test_private_member_hidden_entirely(self, client, keluarga) -> None:
        """Anggota yang menyetel privat tidak muncul sama sekali, bukan
        muncul dengan data kosong (kontrak API)."""
        client.put(
            "/api/v1/settings/visibility",
            json={"data_type": "vitals", "visibility": "private"},
            headers=keluarga["ibu"]["headers"],
        ...
        assert {m["full_name"] for m in members} == {"Ayah"}

    def test_outsider_forbidden(self, client, keluarga) -> None:
        response = client.get(
            f"{FAMILIES}/{keluarga['family']['id']}/dashboard",
            headers=keluarga["luar"]["headers"],
        )
    ...
    def test_member_without_readings_still_listed(
        ...
        baru = register(client, "baru@x.com", "Baru")
        client.post(
            f"{FAMILIES}/join",
            json={"invite_code": keluarga["family"]["invite_code"]},
            headers=baru["headers"],
        )
        members = client.get(
            f"{FAMILIES}/{keluarga['family']['id']}/dashboard",
            headers=keluarga["ayah"]["headers"],
        ).json()["members"]
        entry = next(m for m in members if m["full_name"] == "Baru")
        assert entry["latest"] == []
